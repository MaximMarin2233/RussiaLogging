  ...
  try {
    ...
    const [balanceRows]: any = await db.query(
      `
        SELECT
            DATE(date) AS date,
            SUM(IFNULL(cash_value,0) + IFNULL(bank_value,0)) AS total_amount
        FROM server1.log_action
    ...
    const [operationsRows]: any = await db.query(
      `
        SELECT
            la.id,
            la.date,
        ...
        WHERE la.char_id = ?
        AND (la.cash_value IS NOT NULL OR la.bank_value IS NOT NULL)
    ...
    return NextResponse.json({
      balanceWeek: balanceRows || [],
      lastOperations: operationsRows || []
    })
